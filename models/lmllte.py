...
class LMLLTE(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None, hypernet_spec=None,
        ...
        self.freq = nn.Conv2d(self.feat_dim, hidden_dim, 3, padding=1)
        self.coord_fc = nn.Linear(2, imnet_spec['args']['hidden_dim'], bias=False) #(b*q,2) -> (b*q,c/2)
        ...
        if hypernet_spec is not None:
            ...
            if self.mod_input:
                ...
                self.mod_freq_dim = imnet_spec['args']['hidden_dim'] #一半
        ...
        if imnet_spec is not None:
            if self.mod_input:
                self.imphase = nn.Linear(2, imnet_spec['args']['hidden_dim'], bias=False)
    ...
    def gen_modulations(self, coef, freq, cell=None):
        ...
        freq = torch.sqrt(torch.tensor(2.0))*torch.sin((freq+0.25)*np.pi)
    ...
    def query_rgb(self, coef, freq, mod, coord, cell=None, batched=False):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                inp = torch.sqrt(torch.tensor(2.0))*torch.sin((inp+0.25)*np.pi)
